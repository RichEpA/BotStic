...
iimport telebot   # Импортируем модуль telebot для работы с Telegram-ботом
from PIL import Image   # Импортируем модуль PIL для работы с изображениями
import io   # Импортируем модуль io для работы с байтовыми данными

# Создание экземпляра бота
bot = telebot.TeleBot('YOUR_TELEGRAM_BOT_TOKEN')   # Вставьте сюда токен вашего Telegram-бота

pack_name = ""  # Переменная для хранения имени стикерпака
emojis = []  # Список для хранения эмодзи

# Обработчик команды /newpack
@bot.message_handler(commands=['newpack'])
def new_pack(message):
    global pack_name, emojis
    pack_name = ""  # Сбрасываем имя стикерпака
    emojis = []  # Сбрасываем список эмодзи
    bot.send_message(message.chat.id, 'Введите имя стикерпака')
    bot.register_next_step_handler(message, get_pack_name)
...
def get_pack_name(message):
    global pack_name
    pack_name = message.text
    bot.send_message(message.chat.id, 'Введите эмодзи для стикерпака (от 3 до 5, каждое на новой строке)')
    bot.register_next_step_handler(message, get_pack_emojis)

# Обработчик получения эмодзи для стикерпака
def get_pack_emojis(message):
    global emojis
    text = message.text
    emojis = text.split('\n')
    if len(emojis) >= 3 and len(emojis) <= 5:
        bot.send_message(message.chat.id, 'Отправьте фото для первого эмодзи')
        bot.register_next_step_handler(message, add_sticker_to_pack, emoji_index=0)
    else:
        bot.send_message(message.chat.id, 'Количество эмодзи должно быть от 3 до 5. Попробуйте снова.')
        bot.register_next_step_handler(message, get_pack_emojis)

# Обработчик добавления стикера в стикерпак
def add_sticker_to_pack(message, emoji_index):
    sticker_content = message.photo[0].file_id  # ID фото, присланного пользователем
    sticker_file = bot.get_file(sticker_content)  # Получаем объект файла
    sticker_bytes = bot.download_file(sticker_file.file_path)  # Скачиваем файл в виде байтов
    sticker_image = Image.open(io.BytesIO(sticker_bytes))  # Открываем изображение

    # Меняем размер изображения до 512x512
    sticker_image = sticker_image.resize((512, 512))

    # Добавляем стикер в стикерпак
    bot.add_sticker_to_set(
        user_id=message.from_user.id,
        name=pack_name,
        png_sticker=sticker_image,
        emojis=emojis[emoji_index]
    )

    emoji_index += 1

    if emoji_index < len(emojis):
        bot.send_message(message.chat.id, 'Отправьте фото для следующего эмодзи')
        bot.register_next_step_handler(message, add_sticker_to_pack, emoji_index)
    else:
        bot.send_message(message.chat.id, 'Стикерпак успешно создан. Введите /close для завершения процесса.')
...
def close_command(message):
    ...
    pack_name = ""  # Сбрасываем имя стикерпака
    emojis = []  # Сбрасываем список эмодзи
    bot.send_message(message.chat.id, 'Процесс завершен.')
